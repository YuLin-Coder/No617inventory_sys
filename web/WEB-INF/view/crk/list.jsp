		...
		<main>
			...
			<div id="body">
				<main>
					<div class="main">
						<div class="search">
							<form class="form-inline float-left" action="CrkServlet?action=CrkList"
								method="post">
								<div class="form-group">
									<input type="text" name="key" value="${key}"
										class="form-control" placeholder="负责人">
								</div>
								...
								<tbody>
									...
									<c:forEach items="${list}" var="data">
										<tr>
											...
											<td>
												<c:if test="${data.type =='1'}">入库</c:if>
												<c:if test="${data.type =='2'}">出库</c:if>
											</td>
											<td>${data.charger }</td>
											<td>${data.nums }</td>
											...
											<%--<a class="btn btn-primary btn-sm"
												href="CrkServlet?action=toUpdateCrk&id=${data.id }">修改</a>--%>
	...
	<script>
	  ...
	   $('a.delete').click(function(e) 
   		...
   		            var id = $(e.currentTarget).parent().find('.weui-input').attr('id');
			 		 ...
			 		 　 $.ajax({
						    cache:true,
							url:"CrkServlet?action=deleteCrk",
						  	data:{"id": id},
							type:"post",
						    async:false,
