				...
				<main>
					<div class="main">
								...
								<div class="col-5">
									...
									<input type="hidden" name="id" value="${data.id }">
										...
										<div class="form-group">
											<label>过期商品名称</label> <input type="text" name="name" value="${data.name }"
										...
										<div class="form-group">
											<label>记录入</label> <input type="text" name="jlr" value="${data.jlr }"
																	  class="form-control"> <small
												class="form-text text-muted"></small>
										</div>
										...
										<div class="form-group">
											<label>过期商品类别</label> <select name="cid" class="form-control">
						...
						<div class="submit-box">
							...
							<a type="button" href="GqspServlet?action=GqspList" class="btn btn-outline-secondary">返回</a>
						</div>
					</div>
				</main>
			</div>
	...
	<script>
		$("#save").click(function() {
			$.ajax({
				cache : true,
				type : "post",
				url : "GqspServlet?action=updateGqsp",
				data : $("#saveForm").serialize(),
				async : false,
				success : function(e) {
					if (e == 'yes') {
						alert("修改成功！");
						window.parent.location.href = "GqspServlet?action=GqspList";
					} else {
						alert("修改失败！");
					}
				}
