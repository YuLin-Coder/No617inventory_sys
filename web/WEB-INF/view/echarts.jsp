...
<script type="text/javascript">
 ...
 $.ajax({
        cache : true,
        type : "post",
        url : "GoodsServlet?action=selectCrkCkStatic",
        async : false,
        success : function(e) {
            if (e) {
                var list = eval(e);
     ...
     $.ajax({
			cache : true,
			type : "post",
         url : "GoodsServlet?action=selectGoodsStatic",
			async : false,
			success : function(e) {
				if (e) {
                    var list = eval(e);
    ...
    $.ajax({
        cache : true,
        type : "post",
        url : "GoodsServlet?action=selectCrkRkStatic",
        async : false,
        success : function(e) {
            if (e) {
                var list = eval(e);
                ...
                if(list.length > 0){
                    ...
                    for(var i= 0;i<list.length;i++){
                        message.push(list[i].message);
                        counts.push({value: list[i].counts, name: list[i].message});
                    }
